...
class MailKite:
    ...
    # --- Messages & deliveries -------------------------------------------
    def listMessages(self, before=None, limit=None, search=None):
        path = "/api/messages"
        params = []
        if before is not None:
            params.append("before=%s" % urllib.parse.quote(str(before)))
        if limit is not None:
            params.append("limit=%s" % urllib.parse.quote(str(limit)))
        if search is not None:
            params.append("search=%s" % urllib.parse.quote(str(search)))
        if params:
            path += "?" + "&".join(params)
    ...
    def semanticSearch(self, query):
        """Semantic search over the MailKite docs. ``query`` is a search string;
        returns ``{query, matches}`` where each match is a dict with ``slug``,
        ``title`` and optional ``section``, ``snippet`` and ``score``. PUBLIC —
        no API key required."""
        return self.request("GET", "/v1/docs/search?query=%s" % urllib.parse.quote(query))
    ...
    def listListContacts(self, id, before=None, limit=None):
        path = "/api/lists/%s/contacts" % id
        params = []
        if before is not None:
            params.append("before=%s" % urllib.parse.quote(str(before)))
        if limit is not None:
