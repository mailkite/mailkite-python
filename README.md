...
<h1 align="center">MailKite for Python</h1>
...
<p align="center">
  ...
  <br>The official <a href="https://mailkite.dev">MailKite</a> library for Python.
</p>
...
<p align="center"><a href="https://pypi.org/project/mailkite-dev/"><img src="https://img.shields.io/pypi/v/mailkite-dev?color=2563eb&label=PyPI" alt="PyPI"></a></p>

> **Read-only mirror.** This repo is a generated, release-time mirror of the MailKite monorepo (the private source of truth) — development doesn't happen here. Install from PyPI and open issues against the [MailKite docs](https://mailkite.dev/docs).
...
| --- | --- |
| [`examples/01_send_email.py`](examples/01_send_email.py) | Send an email over a verified domain — the 10-second "it works". |
| [`examples/02_receive_webhook.py`](examples/02_receive_webhook.py) | Receive inbound email as a webhook — and VERIFY the HMAC signature before trusting it. |
| [`examples/03_agent_email_reply.py`](examples/03_agent_email_reply.py) | An AI email agent: inbound email → Claude drafts a reply → MailKite sends it, threaded. |
| [`examples/04_give_your_agent_an_inbox.py`](examples/04_give_your_agent_an_inbox.py) | Give your agent its own email address — MailKite's built-in inbox agent answers mail for… |
| [`examples/05_server_login.py`](examples/05_server_login.py) | Server-side login + register. |
...
| MailKite for Python **(this repo)** | [`mailkite-python`](https://github.com/mailkite/mailkite-python) | PyPI |
